...
window.saveCompanySettings=function(){["companyName","address","phone","email","vat"].forEach(k=>data.settings[k]=document.getElementById("set-"+(k==="companyName"?"company":k))?.value||"");save();notify("Company details saved");render()};
  ...
  function spreadsheetPartyOptions(type, selected) {

    let source = type === "sales"
      ? (data.customers || [])
      : (data.suppliers || []);

    let html = `<option value="">Select ${type === "sales" ? "Customer" : "Supplier"}</option>`;

    source.forEach(r => {

      let name = r.name || r.code || "";
  ...
  function spreadsheetItemOptions(selected) {
    ...
    (data.items || []).forEach(r => {
      ...
      html += `
        <option
          value="${esc(r.name || r.code || "")}"
          data-code="${esc(r.code || "")}"
          data-brand="${esc(r.brand || "")}"
          data-unit="${esc(r.unit || "pcs")}"
          data-rate="${esc(r.rate || 0)}"
          data-vat="${esc(r.vat || 0)}"
          ${String(selected || "") === String(r.name || "") ? "selected" : ""}
        >
  ...
  window.addSpreadsheetRow = function (row = null) {

    sheetState.rows.push(
      Object.assign(blankSheetRow(), row || {})
    );
  ...
  window.deleteSpreadsheetRow = function (index) {

    if (sheetState.rows.length <= 1) {

      sheetState.rows = [blankSheetRow()];

    } else {

      sheetState.rows.splice(index, 1);
  ...
  window.spreadsheetItemChanged = function (index, select) {

    let option = select.options[select.selectedIndex];

    let row = sheetState.rows[index];

    row.item = select.value;
    row.code = option?.dataset.code || "";
    row.brand = option?.dataset.brand || "";
    row.unit = option?.dataset.unit || "pcs";
    row.rate = Number(option?.dataset.rate || 0);
    row.vat = Number(option?.dataset.vat || 0);
  ...
  window.updateSpreadsheetCell = function (index, key, value) {
    ...
    if (
      ["qty","rate","discount","vat"].includes(key)
    ) {
    ...
    calculateSheetRow(row);

    renderSpreadsheetTotals();
  ...
  function renderSpreadsheetRows() {
    ...
    tbody.innerHTML = sheetState.rows.map((row, index) => {
      ...
      return `
        <tr>
          ...
          <td>
            <select
              class="sheet-item"
              onchange="spreadsheetItemChanged(${index},this)"
            >
              ${spreadsheetItemOptions(row.item)}
            </select>
          </td>
          ...
          <td>
            <input
              value="${esc(row.code || "")}"
              onchange="updateSpreadsheetCell(${index},'code',this.value)"
              placeholder="Code"
            >
          </td>
          ...
          <td>
            <input
              value="${esc(row.brand || "")}"
              onchange="updateSpreadsheetCell(${index},'brand',this.value)"
              placeholder="Brand"
            >
          </td>
          ...
          <td>
            <input
              ...
              step="any"
              value="${row.qty}"
              oninput="updateSpreadsheetCell(${index},'qty',this.value)"
            >
          </td>

          <td>
            <input
              value="${esc(row.unit || "pcs")}"
              onchange="updateSpreadsheetCell(${index},'unit',this.value)"
            >
          </td>
          ...
          <td>
            <input
              ...
              step="any"
              value="${row.rate}"
              oninput="updateSpreadsheetCell(${index},'rate',this.value)"
            >
          </td>
          ...
          <td>
            <input
              ...
              step="any"
              value="${row.discount}"
              oninput="updateSpreadsheetCell(${index},'discount',this.value)"
            >
          </td>
          ...
          <td>
            <input
              ...
              step="any"
              value="${row.vat}"
              oninput="updateSpreadsheetCell(${index},'vat',this.value)"
            >
          </td>
          ...
          <td>
            <button
              ...
              onclick="deleteSpreadsheetRow(${index})"
            >
              Delete
            </button>
          </td>
  ...
  function getSheetTotals() {
    ...
    sheetState.rows.forEach(row => {
      ...
      let calc = calculateSheetRow(row);
  ...
  function renderSpreadsheetTotals() {

    let totals = getSheetTotals();

    let sub = document.getElementById("sheet-subtotal");
    let dis = document.getElementById("sheet-discount");
    let vat = document.getElementById("sheet-vat");
    let total = document.getElementById("sheet-grand-total");

    if (sub) sub.textContent = money(totals.subtotal);
    if (dis) dis.textContent = money(totals.discount);
    if (vat) vat.textContent = money(totals.vat);
    if (total) total.textContent = money(totals.grandTotal);
  ...
  function spreadsheetPage(type) {

    sheetState.type = type;

    if (!sheetState.rows.length) {

      sheetState.rows = [blankSheetRow()];

    }

    let isSales = type === "sales";

    let bill = nextNumber(type);
      ...
      <div class="wrap">
        ...
        <div class="head">
          ...
          <div>
            ...
            <div class="page-note">
              ${isSales
                ? "Enter multiple sales items in one invoice."
                : "Enter multiple purchase items in one bill."}
            </div>
        ...
        <div class="panel">
          ...
          <div class="sheet-header-grid">
            ...
            <label>
              ...
              <select
                id="sheet-party"
                onchange="spreadsheetPartyChanged(this)"
              >

                ${spreadsheetPartyOptions(type, "")}
            ...
            <label>
              ...
              ${isSales ? "Address" : "Supplier Address"}
        ...
        <div class="panel">
          ...
          <div class="spreadsheet-toolbar">
            ...
            <h3>
              ${isSales ? "Sales Items" : "Purchase Items"}
            </h3>
        ...
        <div class="panel">
          ...
          <div class="sheet-total-box">
            ...
            <div>
              <span>Subtotal</span>
              <strong id="sheet-subtotal">Rs. 0</strong>
            </div>

            <div>
              <span>Total Discount</span>
              <strong id="sheet-discount">Rs. 0</strong>
            </div>

            <div>
              <span>Total VAT</span>
              <strong id="sheet-vat">Rs. 0</strong>
            </div>

            <div class="grand-total">
              <span>Grand Total</span>
              <strong id="sheet-grand-total">Rs. 0</strong>
            </div>
        ...
        <div class="panel">
          ...
          <div class="form-actions">
            ...
            <button
              class="btn success"
              onclick="saveSpreadsheetVoucher('${type}')"
            >
              Save ${isSales ? "Sales Invoice" : "Purchase Bill"}
            </button>


            <button
              class="btn outline"
              onclick="clearSpreadsheetVoucher()"
            >
              Clear
            </button>
            ...
            <button
              class="btn"
              onclick="printSpreadsheetVoucher('${type}')"
            >
              Print Voucher
            </button>
  ...
  window.spreadsheetPartyChanged = function (select) {

    let option = select.options[select.selectedIndex];

    let mobile = document.getElementById("sheet-mobile");
    let address = document.getElementById("sheet-address");

    if (mobile)
      mobile.value = option?.dataset.mobile || "";
  ...
  /* ---------- Save Spreadsheet Voucher ---------- */

  window.saveSpreadsheetVoucher = function (type) {

    if (!guard("edit","save records"))
      return;

    let bill =
      document.getElementById("sheet-bill")?.value.trim();

    let date =
      document.getElementById("sheet-date")?.value || today();

    let party =
      document.getElementById("sheet-party")?.value.trim();

    let mobile =
      document.getElementById("sheet-mobile")?.value.trim();

    let address =
      document.getElementById("sheet-address")?.value.trim();

    let payment =
      document.getElementById("sheet-payment")?.value || "Cash";
    ...
    let duplicate =
      (data[type] || []).some(r =>
        String(r.bill || "").toLowerCase() ===
        String(bill || "").toLowerCase()
      );
    ...
    if (duplicate) {
      ...
      alert(
        "Duplicate Bill / Invoice No. found: " + bill
      );
    ...
    validRows.forEach((row, index) => {
      ...
      if (type === "sales") {
        ...
        data.transactions.push({
          ...
          note: "Purchase spreadsheet entry"
    ...
    notify(
      `${type === "sales" ? "Sales Invoice" : "Purchase Bill"} ${bill} saved successfully`
    );
  ...
  window.clearSpreadsheetVoucher = function () {
  ...
  window.printSpreadsheetVoucher = function (type) {

    let totals = getSheetTotals();

    let bill =
      document.getElementById("sheet-bill")?.value || "";

    let date =
      document.getElementById("sheet-date")?.value || today();

    let party =
      document.getElementById("sheet-party")?.value || "";


    let rows = sheetState.rows
      .filter(r => r.item)
      .map((r, i) => {

        let c = calculateSheetRow(r);
    ...
    let html = `
      ...
      <head>
        ...
        <title>${type === "sales" ? "Sales Invoice" : "Purchase Bill"}</title>
      ...
      <body>
        ...
        <div class="header">
          ...
          <h1>
            ${esc(
              ...
              "EKIMA ENTERPRISES PVT. LTD."
            )}
          </h1>

          <div>
            ${esc(data.settings.address || "")}
          </div>
        ...
        <div class="meta">
          ...
          <div>
            <b>${type === "sales" ? "Invoice No" : "Bill No"}:</b>
            ${esc(bill)}
          </div>
          ...
          <div>
            <b>Payment:</b>
            ${esc(
              document.getElementById("sheet-payment")?.value ||
              "Cash"
            )}
          </div>
  ...
  css.textContent = `
    ...
    .sheet-header-grid input,
    .sheet-header-grid select{
      width:100%;
      padding:9px;
    ...
    .spreadsheet-table .sheet-item{
      min-width:220px;
    }

    .spreadsheet-table .sn{
      text-align:center;
      font-weight:bold;
    }
  ...
  /* =========================================================
     IMPORTANT:
     Override only Sales & Purchases render.
     Other ERP modules continue using existing renderEnhanced().
     ========================================================= */
  ...
  /* ---------- Make spreadsheet row functions available ---------- */

  window.renderSpreadsheetRows = renderSpreadsheetRows;
